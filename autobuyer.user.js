...
(function () {
    ...
    window.listPlayerFromWatchList = function(t, playerX, sellPriceX){
        setTimeout(function () {
    ...
    window.lytStartSearch = function(){
        writeToDebugLog("lytStartSearch");
    ...
    window.UTAutoBuyerViewController.prototype.init = function init() {
        if (!this.initialized) {
            ...
            view.addTarget(this, this._eResetSelected, UTMarketSearchFiltersView.Event.RESET),
                view.addTarget(this, window.lytStartSearch, UTMarketSearchFiltersView.Event.SEARCH),
    ...
    window.processor = window.setInterval(function () {
        ...
        if (window.autoBuyerActive) {
            ...
            var time = (new Date()).getTime();
            //search market with certain filter
            ...
            //writeToLog("processors deactive");
        ...
        services.Item.searchTransferMarket(searchCriteria, window.currentPage).observe(this, (function (sender, response) {
            if (response.success && window.autoBuyerActive) {
                ...
                **/
                ...
                writeToDebugLog("response.data.items.length:" + response.data.items.length);
                var buyNumber = 1;
                for (let i = 0; i < response.data.items.length; i++) {
                    ...
                    if (rating_ok && window.autoBuyerActive && buyNowPrice <= userBuyNowPrice && buyNowPrice <= window.futStatistics.coinsNumber && !window.bids.includes(auction.tradeId)) {
                        ...
                        if (player._auction._bidState === "none" && player._auction._tradeState === "active"){
                            (function (t, playerX, checkPriceX, sellPriceX) {
                                setTimeout(function () {
                                    let player_nameX = window.getItemName(playerX);
                                    let auction = playerX._auction;
    ...
    window.relistExpiredPlayers = function(t, playerX, sellPriceX){
        setTimeout(function () {
            let player_nameX = window.getItemName(playerX);
...
**/
    ...
    window.unWatchPlayer = function(player){
    ...
    window.clearSoldItems = function (soldItems) {
        services.Item.clearSoldItems().observe(this, function (t, response) {
            if (response.success){
                writeToDebugLog("clearSoldItems success");
                //refresh clear sor
                services.Item.refreshAuctions(soldItems).observe(this, function (t, refreshResponse) {});
            }else{
                writeToDebugLog("clearSoldItems fail");
            }
        });
    }
})();
